...
if [ "$1" == "notarize" ]; then
	...
	# release build name
	RELEASE_BUILD="iowolfmp.app"

	# Pre-notarized zip file (not what is shipped)
	PRE_NOTARIZED_ZIP="iortcwmp_prenotarized.zip"

	# Post-notarized zip file (shipped)
	POST_NOTARIZED_ZIP="iortcwmp_notarized.zip"

	BUNDLE_ID="com.macsourceports.iortcwmp"
	...
	ENTITLEMENTS_FILE="misc/xcode/iortcw/iortcw.entitlements"
